...
SpikeDetect::SpikeDetect(void) : DefaultGUIModel("Spike Detector", ::vars, ::num_vars), 
                                 thresh(-0.02), min_int(5e-3), last_spike(0), 
                                 count(0), state(0) 
{
	setWhatsThis(
...
void SpikeDetect::execute(void)
{
	...
	case 2:
		if (vm > thresh && (time - last_spike) > 100) state = 4;
		else if (vm < thresh) state = 3;
		break;
	...
	case 4:
		if (vm < thresh) state = -1;
		break;

	case -1:
		if (time - last_spike > min_int) state = 0;
		break;
